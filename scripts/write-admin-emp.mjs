...
// ============================================================
// ADMIN EMPLOYEES TABLE - Full CRUD with auth account creation
// ============================================================
writeFileSync(src('components/admin/AdminEmployeesTable.tsx'), `
  ...
  const FormFields = ({ showPwd = false }: { showPwd?: boolean }) => (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div className="sm:col-span-2 flex items-center gap-2">
        <input type="checkbox" id="is_active" checked={form.is_active} onChange={e=>set("is_active",e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-blue-600" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <Button onClick={openCreate} className="bg-blue-600 hover:bg-blue-700 text-white text-sm">
      ...
      {/* Table */}
      {loading ? (
        <div className="text-center py-12"><div className="w-7 h-7 border-4 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto" /></div>
      ) : (
                ...
                <tr key={emp.id}>
                  ...
                  <td>
                    <span className="text-xs text-amber-600 font-medium">
                      {emp.total_ratings > 0 ? \`★ \${Number(emp.rating).toFixed(1)} (\${emp.total_ratings})\` : "—"}
                    </span>
                  </td>
                  <td>
...
console.log('✓ AdminEmployeesTable.tsx written');
