...
const src = (p) => resolve(process.cwd(), 'src', p);

// ============================================================
// EMPLOYEE DASHBOARD PAGE
// ============================================================
writeFileSync(src('pages/EmployeeDashboard.tsx'), `
import { useEffect, useState, useRef } from "react";
import { useNavigate } from "react-router-dom";
import {
  ClipboardList, CheckCircle, Clock, AlertCircle, Camera,
  Upload, X, User, MapPin, Phone, Mail, ChevronDown, ChevronUp, Eye
} from "lucide-react";
...
const EmployeeDashboard = () => {
  ...
  const handlePhotoUpload = async (complaintId: string, file: File) => {
    if (!user) return;
    ...
    const { error: dbErr } = await supabase.from("complaints").update({
      resolved_photo_url: urlData.publicUrl,
      status: "resolved",
      resolved_at: new Date().toISOString(),
    }).eq("id", complaintId);
    if (dbErr) { toast({ title: "Error", description: dbErr.message, variant: "destructive" }); }
    else {
      setComplaints(prev => prev.map(c => c.id === complaintId ? { ...c, resolved_photo_url: urlData.publicUrl, status: "resolved", resolved_at: new Date().toISOString() } : c));
      toast({ title: "Photo uploaded!", description: "Complaint marked as resolved with photo evidence." });
    }
    setUploadingId(null);
  };
  ...
  const filtered = complaints.filter(c => {
    if (activeTab === "active") return ["pending", "in_progress"].includes(c.status);
                        ...
                        {c.status !== "resolved" && c.status !== "rejected" && (
                          <div className="flex flex-wrap gap-2">
                            ...
                            <label className={\`nmc-btn-primary text-xs cursor-pointer \${uploadingId === c.id ? "opacity-60 pointer-events-none" : ""}\`}>
                              {uploadingId === c.id ? (
                                <span className="flex items-center gap-1"><div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" /> Uploading...</span>
                              ) : (
                              ...
                              <input
                                ...
                                onChange={async (e) => {
                                  ...
                                  if (f) await handlePhotoUpload(c.id, f);
                                }}
                              />
                            </label>
                          </div>
...
console.log('✓ EmployeeDashboard.tsx written');
