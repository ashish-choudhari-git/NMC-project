-- ============================================================
-- NAGPUR MUNICIPAL CORPORATION (NMC) — MASTER MIGRATION
-- File: 001_master_migration.sql
...
CREATE INDEX IF NOT EXISTS idx_complaints_deadline          ON public.complaints(deadline);
CREATE INDEX IF NOT EXISTS idx_complaints_assigned_employee ON public.complaints(assigned_employee_id);
CREATE INDEX IF NOT EXISTS idx_complaints_assignment_type   ON public.complaints(assignment_type);
...
-- ============================================================
-- SECTION 5: HELPER FUNCTION (needed before RLS policies)
-- ============================================================
...
-- complaint_assignments policies
DROP POLICY IF EXISTS "Anyone can view complaint assignments" ON public.complaint_assignments;
CREATE POLICY "Anyone can view complaint assignments" ON public.complaint_assignments
  FOR SELECT USING (true);
...
-- Auto-assign complaint to best matching employee
...
SELECT
    COUNT(*) FILTER (WHERE c.status = 'pending')                          AS pending_complaints,
    COUNT(*) FILTER (WHERE c.status = 'resolved')                         AS resolved_complaints,
    COUNT(*) FILTER (WHERE c.status = 'rejected')                         AS rejected_complaints,
    COUNT(*) FILTER (WHERE c.status = 'in_progress')                      AS in_progress_complaints,
    COUNT(*) FILTER (WHERE c.deadline < now()
                       AND c.status NOT IN ('resolved','rejected'))        AS overdue_complaints,
    COUNT(*) FILTER (WHERE c.created_at >= CURRENT_DATE)                  AS today_complaints,
    COUNT(*) FILTER (WHERE c.created_at >= date_trunc('week', now()))     AS week_complaints,
    COUNT(*) FILTER (WHERE c.created_at >= date_trunc('month', now()))    AS month_complaints,
    ...
    (SELECT COUNT(*) FROM public.events WHERE is_approved = false)         AS pending_events,
    (SELECT COUNT(*) FROM public.events WHERE is_approved = true)          AS approved_events,
    (SELECT COUNT(*) FROM public.employees WHERE is_active = true)         AS active_employees,
    (SELECT COUNT(*) FROM public.employees WHERE is_active = false)        AS inactive_employees
...
-- ============================================================
-- SECTION 11: SEED — COMPLAINT CATEGORIES (12 total)
-- ============================================================
...
-- ============================================================
-- SECTION 12: SEED — EMPLOYEES (200+ total)
-- ============================================================
...
BEGIN
  ...
  VALUES (gen_random_uuid(), 'nmc@gmail.com', v_admin_id, jsonb_build_object('sub', v_admin_id::text, 'email', 'nmc@gmail.com'), 'email', now(), now(), now())
  ON CONFLICT DO NOTHING;
  INSERT INTO public.profiles  (user_id, first_name, last_name, email) VALUES (v_admin_id, 'NMC', 'Admin', 'nmc@gmail.com') ON CONFLICT DO NOTHING;
  INSERT INTO public.user_roles(user_id, role) VALUES (v_admin_id, 'admin') ON CONFLICT DO NOTHING;
...
-- Employee accounts (default password: pass@123)
DO $$
DECLARE
  emp       RECORD;
  v_uid     UUID;
...
BEGIN
  ...
  LOOP
    ...
    UPDATE public.employees SET user_id = v_uid WHERE id = emp.id;
    INSERT INTO auth.identities (id, provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at)
    VALUES (gen_random_uuid(), lower(trim(emp.email)), v_uid, jsonb_build_object('sub', v_uid::text, 'email', lower(trim(emp.email))), 'email', now(), now(), now())
    ON CONFLICT DO NOTHING;
    INSERT INTO public.user_roles (user_id, role) VALUES (v_uid, 'employee') ON CONFLICT DO NOTHING;
...
-- Employee login: <emp@nmc.gov.in> / pass@123
-- Citizen login:  Self-register via app
-- ============================================================
...
-- Step 1: Saare manually-inserted employee auth users delete karo
-- (admin nmc@gmail.com safe rahega)
DELETE FROM auth.identities
WHERE user_id IN (
  SELECT u.id FROM auth.users u
  WHERE u.email != 'nmc@gmail.com'
    AND u.email LIKE '%@nmc.gov.in'
);

DELETE FROM auth.users
WHERE email != 'nmc@gmail.com'
  AND email LIKE '%@nmc.gov.in';

-- Step 2: Employees table me user_id unlink karo
UPDATE public.employees SET user_id = NULL;

-- Step 3: Employee roles clear karo
DELETE FROM public.user_roles WHERE role = 'employee';

-- Verify
SELECT COUNT(*) as auth_users_remaining FROM auth.users;
SELECT COUNT(*) as employees_without_uid FROM public.employees WHERE user_id IS NULL;
